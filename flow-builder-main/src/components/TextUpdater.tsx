...
// Define the shape of the custom data with an index signature
interface TextUpdaterNodeData {
    label?: string; // Define label as an optional string
    [key: string]: unknown; // Satisfies Record<string, unknown>
}

// Define the custom node type, extending the Node type
type TextUpdaterNode = Node<TextUpdaterNodeData>;

// Use TextUpdaterNode as the generic for NodeProps
export function TextUpdaterNode({ data, id }: NodeProps<TextUpdaterNode>) {
        ...
        <div className="text-updater-node relative w-[160px] overflow-hidden rounded-md bg-white shadow-2xl">
            ...
            <div className="flex flex-col text-xs">
                ...
                <input
                    id={`text-${id}`}
                    name="text"
                    value={data.label ?? ''} // TypeScript knows data.label is string | undefined
                    onChange={onChange}
